...
7. When the user requests fixes, resume the same Claude Session, run verification, create an additional local fix commit, and rerun native `code-review` against the original fixed point.
...
Capture a fixed point, implement and verify, create the local review checkpoint commit, run native `code-review`, and apply only user-requested fixes through the same Session. Do not create or update a Ticket.
